...
| K8s：Deployment / Service / Ingress | ✅ 完成 | 企業容器編排標準 |
| K8s：rolling update / health check | ✅ 完成 | 零停機部署 |
| K8s：Application-level HA（replicas + probe） | ✅ 完成 | Pod 失效自動重建、流量切換 |
| Retry with Exponential Backoff | ✅ 完成 | 分散式系統韌性基礎（Availability Tactic） |
| Correlation ID / 分散式追蹤 | ✅ 完成 | 跨服務 request 追蹤，Observability 基礎 |
| asyncio / async 非同步設計 | ✅ 完成 | FastAPI / Python async 框架的正確使用方式 |
...
## Retry with Exponential Backoff（Branch 2b）

**它是什麼：** 請求失敗時自動重試，且每次等待時間指數增長，避免瞬間大量重試把下游打死。  
**為什麼用它：** 分散式系統中服務暫時不可用（pod 重啟、網路抖動）是常態，Retry 是最基本的韌性設計（Availability Tactic）。
...
等待時間序列：1s → 2s → 4s（上限 10s）。失敗 3 次才真的拋出 exception。

**演練驗證：** `kubectl delete pod search-0` 刪掉 search pod，orchestrator 在 search 重啟期間（約 12 秒）自動重試，最終仍回傳正確答案，使用者端完全無感。

**學習後的體會：**  
tenacity 的 `@retry` 裝飾器直接加在 async 函式上就能用，不需要另外引入 `AsyncRetrying`。重試只適合冪等操作（相同參數重試多次結果一樣），search 是純讀取，天然冪等。如果是寫入操作（如付款），重試前必須先確認冪等性設計，否則會重複執行。

---

## Correlation ID / 分散式追蹤（Branch 2b）

**它是什麼：** 每個進來的 request 分配一個唯一 ID（UUID），透過 HTTP header（`X-Request-ID`）傳給所有下游服務，每個服務的 log 都帶這個 ID。  
**為什麼用它：** 單一服務的 log 好查，但跨四個服務的請求要追蹤時，沒有 Correlation ID 根本找不到哪些 log 屬於同一個請求。

```python
# orchestrator middleware：自動產生或接收 X-Request-ID
@app.middleware("http")
...
await client.post(f"{search_url}/search", headers=headers, ...)
```
...
[432f82fa] search: k8s          ← search service log
[432f82fa] ITER 1 LLM → TOOL: search(...)
[432f82fa] ITER 2 LLM → TOOL: summarize(...)
[432f82fa] DONE: Kubernetes (k8s) 是一個開源...
```

同一個 `432f82fa` 在四個服務的 log 裡都出現，`kubectl logs` 可以 grep 找出完整的請求路徑。

**和生產環境 Tracing 的關係：**  
Correlation ID 是分散式追蹤的最簡版本，不需要額外工具。生產環境的 Jaeger / Zipkin / AWS X-Ray 在此基礎上增加了時間戳、span 層級、視覺化等功能，但原理相同：每個請求一個 ID，跨服務傳遞。

**學習後的體會：**  
實作完才真正理解為什麼 OpenTelemetry 規範要求所有服務都必須傳遞 trace context。沒有這個，分散式系統的 debug 就是大海撈針——四個服務的 log 混在一起，完全無法判斷哪些行屬於同一個請求。
...
**Branch 2b 完成後：**  
「我把 v2a 的 Docker Compose 部署遷移到 minikube K8s。核心工作是寫 Deployment / StatefulSet / Ingress / ConfigMap / PVC 的 YAML，把 ChromaDB 的有狀態性用 StatefulSet + PVC 處理，其餘無狀態服務用 Deployment。過程中實作了三個分散式系統的基礎能力：Retry with Exponential Backoff（tenacity）、Correlation ID 跨服務追蹤、Liveness / Readiness Probe 分離。最棘手的問題是 orchestrator 在 K8s 下被 liveness probe 誤殺：診斷發現 asyncio event loop 在請求期間被阻塞（health probe 的 access log 消失是關鍵訊號），根因是 sync httpx 在 run_in_executor 中的行為；改用 async httpx + async def 全程 await 解決。完成了 rolling update 和 kill pod 演練，驗證 zero-downtime 更新和自癒能力。」
